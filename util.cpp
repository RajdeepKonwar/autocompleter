/**
 ...
 @section OVERVIEW
 Implementation of Timer and Utils functions.
...
unsigned int Utils::stripFrequency( string &line ) {
  ...
  //! Gets freq as sub-string and converts into an unsigned int using stoi
  unsigned int freq = std::stoi( line.substr( 0, count ) );
  //! Erases frequency from line (by-reference), i.e. line now only has word(s)
  line.erase( 0, count );
...
void Utils::load_vector( vector< string > &vec, istream &words,
  ...
  for( unsigned int i = 0; i < num_words; i++ ) {
    getline( words, line );
    if( words.eof() ) {
      cout << "Warning!  Only " << i << " words read from file." << endl;
      break;
    }
